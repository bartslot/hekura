...
var live = !!document.querySelector(".smooth-scroll"); // pages without the story scroller (vfx.html) skip setup instead of crashing
var jupjup = false;
let mouseDismissed = false;
...
if (live) { 
  ...
  locoScroll.on("scroll", ({ scroll }) => {
    if (mouseDismissed) return;
    if (scroll.y > window.innerHeight * 0.2) {
      mouseDismissed = true;
      gsap.to("#mouse", { duration: 0.2, autoAlpha: 0 });
      gsap.killTweensOf("#mouseWheel");
    }
  });
  ...
  gsap.utils.toArray(".inner-stars>path").forEach(layer => {
    tl.set(layer, { transformOrigin: "center" }, 0);
  });
  gsap.utils.toArray("p").forEach(paragraph => {
    const delayAttr = paragraph.getAttribute("data-delay") || paragraph.getAttribute("delay");
    const startDelay = delayAttr ? parseFloat(delayAttr) / 1000 : 0;
    ...
    ScrollTrigger.create({
      ...
      onEnter: () => {
        ...
        split.chars.forEach((char, idx) => {
          const nearestDelayEl = char.closest("[data-delay],[delay]");
          const rawExtra = nearestDelayEl ? (nearestDelayEl.getAttribute("data-delay") || nearestDelayEl.getAttribute("delay")) : null;
          const extraDelay = rawExtra && !isNaN(parseFloat(rawExtra)) ? parseFloat(rawExtra) / 1000 : 0;
          tlChars.from(char, {
            duration: 0.25,
            opacity: 0,
            y: 6,
            ease: "power2.out"
          }, startDelay + extraDelay + idx * 0.04);
        });
      }
    });
  });
  gsap.utils.toArray(".bubblewrapper").forEach(wrapper => {
    if (wrapper.closest("#comic")) {
      return; // comic bubbles have their own timelines; avoid double transforms
    }
    const delayAttr = wrapper.getAttribute("data-delay");
    const startDelay = delayAttr ? parseFloat(delayAttr) / 1000 : 0;
    const appear = (wrapper.getAttribute("data-appear") || wrapper.getAttribute("data-from") || "left").toLowerCase();
    let fromX = 0;
    let fromY = 0;
    if (appear.includes("left")) fromX = -60;
    if (appear.includes("right")) fromX = 60;
    if (appear.includes("top")) fromY = -60;
    if (appear.includes("bottom")) fromY = 60;
    gsap.from(wrapper, {
      duration: 0.6,
      opacity: 0,
      x: fromX,
  ...
  gsap.utils.toArray(".image.fullscreen").forEach(el => {
  ...
  // Moon drifts slower than the page: centered (y 0) when #clouds fills the screen
  gsap.fromTo(".moon", { y: () => innerHeight * -0.15 }, {
    y: () => innerHeight * 0.15,
    ease: "none",
    scrollTrigger: { trigger: "#clouds", scroller: ".smooth-scroll", start: "top bottom", end: "bottom top", scrub: true, invalidateOnRefresh: true }
  });
  ...
  ScrollTrigger.create({
    ...
    start: "top 90%",
    onEnter: () => {
      if (typeof triggerDanceLoop === "function") triggerDanceLoop();
    },
    onEnterBack: () => {
      if (typeof triggerDanceLoop === "function") triggerDanceLoop();
    },
    onLeave: () => {
      if (typeof stopDanceLoop === "function") stopDanceLoop(true);
    }
  });
  ...
  const comic1 = gsap.timeline({ scrollTrigger: { trigger: ".comic1", scroller: ".smooth-scroll", start:"top +120%", end:"bottom", scrub: true}});
    ...
    comic1.from("#comic .comic1", 1,{autoAlpha:0, delay: .2}).to("#comic .comic1", 1,{y:-200, ease: "power3.inOut"},"<")
  ...
  const comic3 = gsap.timeline({ scrollTrigger: { trigger: ".comic3", scroller: ".smooth-scroll", start:"top +110%", end:"bottom", scrub: true }});
    ...
    comic3.from("#comic .comic3", 1,{y:200, scale: 0.7, delay: .3})
          .from("#comic #bubble3", 1,{autoAlpha: 0.7, delay: .3 }, "<")
          
          .to("#comic .bubblewrapper-3", 1,{y:-400, x:0, autoAlpha:0, ease: "power3.inOut"})
  ...
  const comic4 = gsap.timeline({ scrollTrigger: { trigger: ".comic3", scroller: ".smooth-scroll", start:"top center", end:"bottom", scrub: true }});
    ...
    comic4
        ...
        .from("#comic .comic4, #comic .bubblewrapper-4", 2,{  x:-100}, "<")
        .to("#comic .comic4, #comic .bubblewrapper-4", 2,{ scale: 1.2, y:-200, ease: "power3.inOut"},"<")
  ...
  const ritual = gsap.timeline({ scrollTrigger: { 
    ...
    ritual 
      ...
      .to('#ritual', 10,{scale:2, blur: 3},"3")
      .add( function(){ 
        jupjup = false; 
        if (typeof playRitual === "function") playRitual(); 
        if (typeof stopSilence === "function") stopSilence();
      } )
  ...
  // comic1.to("#comic .image",  2,{autoAlpha:1, y:20, ease: "power3.Out"}),"+=4"})
    ...
    function logo(){
      tl.from ("#logo", 1, { x:5000, opacity: 0, scale: 1}) //inner-stars
        ...
        .add(() => {
          if (!mouseDismissed) {
            gsap.to("#mouse", { duration: 0.4, delay: 4, autoAlpha: 0.8 });
            gsap.to("#mouseWheel", { duration: 0.2, autoAlpha: 1, repeat: -1, repeatDelay: 0.4, y: 7, yoyo: true });
          } else {
